...
def test_exception_has_correct_error_code(exception_class, expected_code):
    """
    Purpose:
        Verifies that each concrete exception class in `app.core.exceptions` exposes the expected `error_code`.
        This matters because the error code is the machine-readable part of the public API error contract.
...
def test_exception_has_correct_http_status(exception_class, expected_status):
    """
    Purpose:
        Verifies that each concrete exception class in `app.core.exceptions` exposes the expected HTTP status code.
        This matters because the status code is part of the API error envelope emitted by the exception handlers.
...
def test_api_exception_base_class_stores_all_parameters(faker):
    """
    Purpose:
        Verifies that `app.core.exceptions.APIException` stores the constructor arguments that drive the API error contract.
        This matters because all concrete exception types inherit this state and the exception handlers serialize it directly.
...
def test_api_exception_with_details_none_defaults_to_empty(faker):
    """
    Purpose:
        Verifies that `app.core.exceptions.APIException` normalizes `details=None` into an empty list.
        This matters because handlers expect a list-shaped details field even when no details are provided.
...
def test_exception_with_details_none_defaults_to_empty_list(exception_class):
    """
    Purpose:
        Verifies that detail-bearing concrete exceptions normalize `details=None` into an empty list.
        This matters because callers should get the same details shape whether they use the base class or concrete subclasses.
...
def test_exception_stores_custom_details(exception_class, message, details):
    """
    Purpose:
        Verifies that detail-bearing concrete exceptions preserve the custom message and detail payload passed at construction.
        This matters because handlers serialize these fields directly into the API response.
...
def test_simple_exception_accepts_custom_message(exception_class, faker):
    """
    Purpose:
        Verifies that positional-message exception classes preserve the custom message passed to their constructor.
        This matters because the exception handlers expose these messages directly to API clients for handled exceptions.
...
def test_parameterized_exception_accepts_custom_message(exception_class, faker):
    """
    Purpose:
        Verifies that keyword-message exception classes preserve the custom message passed to their constructor.
        This matters because these subclasses expose their message through the same API error serialization path as the base class.
...
def test_concrete_exception_is_subclass_of_api_exception(exception_class):
    """
    Purpose:
        Verifies that every concrete exception class subclasses `app.core.exceptions.APIException`.
        This matters because the API exception handler relies on that inheritance chain to catch handled application errors.
