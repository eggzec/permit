...
def test_settings_builds_database_dsn_from_fields(faker) -> None:
    """
    Verifies that `app.core.config.Settings` assembles `DATABASE_DSN` from the
    individual Postgres fields. This matters because database connectivity
    depends on the computed DSN matching the configured environment.
...
def test_settings_expose_default_token_expiries(faker) -> None:
    """
    Verifies that `app.core.config.Settings` exposes the default access-token
    and refresh-token expiry values. This matters because auth token lifetime
    is part of the application's default security contract.
