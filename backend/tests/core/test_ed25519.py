...
def test_load_private_ed25519_key_returns_private_key() -> None:
    """
    Purpose:
        Verifies that `app.core.ed25519.load_private_ed25519_key` accepts a PEM-encoded Ed25519 private key and returns the expected key type.
        This matters because license signing and verification depend on loading the correct asymmetric key material.
...
def test_load_private_ed25519_key_rejects_rsa_private_key() -> None:
    """
    Purpose:
        Verifies that `app.core.ed25519.load_private_ed25519_key` rejects PEM data for a non-Ed25519 private key.
        This matters because callers should fail fast when they provide the wrong key type for Ed25519 operations.
...
def test_load_public_ed25519_key_returns_public_key() -> None:
    """
    Purpose:
        Verifies that `app.core.ed25519.load_public_ed25519_key` accepts a PEM-encoded Ed25519 public key and returns the expected key type.
        This matters because license verification depends on loading the correct public key material.
...
def test_load_public_ed25519_key_rejects_rsa_public_key() -> None:
    """
    Purpose:
        Verifies that `app.core.ed25519.load_public_ed25519_key` rejects PEM data for a non-Ed25519 public key.
        This matters because callers must not accidentally verify signatures with incompatible key material.
