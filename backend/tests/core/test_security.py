...
def test_get_password_hash_round_trip_verifies_password(faker) -> None:
    """
    Verifies that `app.core.security.get_password_hash` and
    `app.core.security.verify_password` work together for the valid-password
    round trip. This matters because the application's password wrapper contract
    depends on a stored hash being accepted for the original plaintext.
...
def test_token_round_trip_preserves_vendor_id_and_type(
    ...
    """
    Verifies that tokens issued by the project helpers decode back to the
    expected vendor id and token type. This matters because the application's
    auth layer depends on a stable round-trip contract for access and refresh
    tokens.
    ...
    Rationale:
        This test exercises the project's token helpers as a boundary round trip
        instead of asserting on JWT-library internals.
