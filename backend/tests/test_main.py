...
def build_fake_connection(error: Exception | None = None) -> SimpleNamespace:
    """
    Builds a connection-like object that records executed statements and can
    raise a configured error.

    Used by:
        - test_lifespan_success_sets_state_and_closes_pool - records the startup
        connectivity query.
        - test_lifespan_connection_failure_closes_pool_and_wraps_error - forces
        the connectivity-check failure path.
...
def build_fake_pool(connection: SimpleNamespace) -> SimpleNamespace:
    ...
    Used by:
        - test_lifespan_success_sets_state_and_closes_pool - supplies the pool
        stored on app state during the successful lifespan path.
        - test_lifespan_connection_failure_closes_pool_and_wraps_error -
        supplies the pool closed after a startup failure.
...
def test_custom_generate_unique_id_uses_expected_prefix(
    ...
    Rationale:
        The route object is represented with a `SimpleNamespace` because the
        function depends only on `tags` and `name`.
...
async def test_lifespan_success_sets_state_and_closes_pool(monkeypatch) -> None:
    """
    Verifies that `app.main.lifespan` stores settings and the database pool
    on `app.state`, performs a startup connectivity check, and closes the pool
    on shutdown. This matters because application startup must both initialize
    shared state and clean up resources reliably.

    Covers:
        - `app.main.lifespan`

    Rationale:
        This test monkeypatches `app.main.Settings` and
        `app.main.ConnectionPool` because startup constructors are not
        injectable in the current design. The fake pool lets the test assert
        observable lifespan behavior without opening a real pool.

    Fixtures:
        monkeypatch: Pytest fixture used to replace `app.main.Settings` and
        `app.main.ConnectionPool` during the test.
    """
    ...
    assert pool.connection_instance.executed == ["SELECT 1"], (
        f"Expected startup connectivity query ['SELECT 1'], got "
        f"{pool.connection_instance.executed}"
    )
    assert pool.close_calls == 1, (
        f"Expected lifespan shutdown to close the pool once, got "
        f"{pool.close_calls}"
    )
...
async def test_lifespan_connection_failure_closes_pool_and_wraps_error(
    ...
    """
    Verifies that `app.main.lifespan` closes the pool and wraps startup
    connection failures in the expected runtime error. This matters because
    startup failures must preserve the root cause while still cleaning up
    ...
    Rationale:
        This test monkeypatches `app.main.Settings` and
        `app.main.ConnectionPool` because startup constructors are not
        injectable in the current design. The fake pool forces the failure path
    ...
    assert exc_info.value.__cause__ is root_error, (
        "Expected lifespan to preserve the original connection error as "
        "__cause__"
    )
    ...
    assert pool.close_calls == 1, (
        f"Expected failed startup to close the pool once, got "
        f"{pool.close_calls}"
    )
