...
def test_create_vendor_returns_created_row(db_conn: Connection, faker) -> None:
    """
    Verifies that `app.crud.vendor.create_vendor` inserts a vendor row and
    returns the created record. This matters because higher-level auth flows
    depend on CRUD creation returning the persisted vendor identity and email.
...
def test_get_vendor_by_email_is_case_insensitive(
    ...
    """
    Verifies that `app.crud.vendor.get_vendor_by_email` performs
    case-insensitive email lookup. This matters because the auth layer should
    treat vendor emails consistently regardless of request casing.
...
def test_create_vendor_returns_none_for_case_insensitive_duplicate(
    ...
    """
    Verifies that `app.crud.vendor.create_vendor` returns `None` when a
    case-insensitive duplicate email already exists. This matters because
    duplicate vendor emails must be rejected consistently even when the casing
    differs.
...
def test_deleted_vendor_is_excluded_from_email_and_id_lookups(
    ...
    """
    Verifies that `app.crud.vendor.get_vendor_by_email` and
    `app.crud.vendor.get_vendor_by_id` ignore soft-deleted vendor rows. This
    matters because deleted vendors must not remain visible to auth and
