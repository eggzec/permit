...
@pytest.mark.unit
def test_gen_activation_code_returns_formatted_crockford_code() -> None:
    """
    Purpose:
        Verifies that `app.services.license_gen.gen_activation_code` returns a four-segment activation code that matches the expected pattern and alphabet.
        This matters because the activation code is a human-facing artifact with a strict formatting contract.

    Covers:
        - `app.services.license_gen.gen_activation_code`

    Rationale:
        The test checks only the observable code format and alphabet membership rather than any internal randomness mechanism.
    ...
    assert ACTIVATION_CODE_PATTERN.fullmatch(code), (
        f"Expected activation code to match {ACTIVATION_CODE_PATTERN.pattern}, got '{code}'"
    )
    assert len(segments) == 4, (
        f"Expected four activation code segments, got {len(segments)} from '{code}'"
    )
    assert all(len(segment) == 4 for segment in segments), (
        f"Expected every activation code segment to have length 4, got {segments}"
    )
    assert all(char in ALPHABET for char in flattened), (
        f"Expected activation code characters to stay within Crockford alphabet, got '{flattened}'"
    )
