...
def test_get_db_yields_cursor_when_pool_exists():
    """
    Purpose:
        Verifies that `app.api.deps.get_db` opens one pooled connection, yields one cursor, and then terminates cleanly.
        This matters because route dependencies rely on `get_db` to provide the database cursor used by downstream logic.
...
def test_get_db_raises_when_pool_missing():
    """
    Purpose:
        Verifies that `app.api.deps.get_db` raises a service-unavailable error when the application state has no database pool.
        This matters because route handlers should fail with a clear infrastructure error instead of dereferencing a missing pool.
...
def test_get_settings_raises_when_settings_missing():
    """
    Purpose:
        Verifies that `app.api.deps.get_settings` fails fast when application settings are absent from `app.state`.
        This matters because auth and configuration-dependent code expects the dependency to provide a valid settings object.
...
def test_get_settings_returns_settings_when_present(app_settings):
    """
    Purpose:
        Verifies that `app.api.deps.get_settings` returns the exact settings object stored on application state.
        This matters because callers rely on configuration identity and values from the dependency rather than a copied object.
...
def test_get_rls_cursor_raises_when_pool_missing(faker):
    """
    Purpose:
        Verifies that `app.api.deps.get_rls_cursor` raises a service-unavailable error when no database pool is configured.
        This matters because authenticated routes still need the underlying pool before row-level-security context can be installed.
...
def test_get_rls_cursor_sets_context_and_yields_cursor(faker):
    """
    Purpose:
        Verifies that `app.api.deps.get_rls_cursor` sets the authenticated vendor context in SQL before yielding the cursor.
        This matters because row-level-security queries depend on the app context being installed for the current vendor.
...
def test_get_current_vendor_id_payload_errors(
    ...
    """
    Purpose:
        Verifies that `app.api.deps.get_current_vendor_id` rejects access tokens whose decoded payload omits or corrupts the vendor id claim.
        This matters because authorization depends on a valid vendor identity being present in every access token.
...
def test_get_current_vendor_id_rejects_invalid_tokens(
    ...
    """
    Purpose:
        Verifies that `app.api.deps.get_current_vendor_id` rejects non-access, expired, and garbage bearer tokens.
        This matters because authenticated endpoints depend on the dependency to enforce token type and expiry rules uniformly.
...
def test_get_current_vendor_id_returns_vendor_id_from_valid_access_token(
    ...
    """
    Purpose:
        Verifies that `app.api.deps.get_current_vendor_id` returns the vendor id claim from a valid signed access token.
        This matters because downstream route code depends on receiving the authenticated vendor identity from the dependency.
    ...
    assert resolved_vendor_id == vendor_id, (
        f"Expected authenticated vendor_id '{vendor_id}', got '{resolved_vendor_id}'"
    )
...
def test_get_current_vendor_id_requires_credentials(app_settings) -> None:
    """
    Purpose:
        Verifies that `app.api.deps.get_current_vendor_id` rejects requests that do not include bearer credentials at all.
        This matters because the dependency is the authentication gate for protected routes.
