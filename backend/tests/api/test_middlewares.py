...
def test_add_request_id_preserves_valid_incoming_header(middleware_app):
    """
    Purpose:
        Verifies that `app.api.middlewares.add_request_id` preserves a valid incoming request id instead of replacing it.
        This matters because upstream systems may supply trace identifiers that should survive through the application.
...
def test_add_request_id_replaces_invalid_header_with_uuid(middleware_app):
    """
    Purpose:
        Verifies that `app.api.middlewares.add_request_id` replaces an invalid incoming request id with a fresh canonical UUID.
        This matters because downstream tracing should not trust malformed client-supplied identifiers.
