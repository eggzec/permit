...
def build_auth_payload(faker, **overrides: str) -> dict[str, str]:
    ...
    Used by:
        ...
        - test_signup_duplicate_email_returns_409: reuses the same signup
        payload to trigger a duplicate conflict.
        - test_signup_validation_errors_return_422: mutates a valid payload into
        invalid auth-route cases.
        - test_login_returns_token_pair: provisions a vendor before exercising
        the login route.
        - test_login_rejects_invalid_credentials: produces the baseline valid
        credentials before one field is varied.
        - test_refresh_issues_new_tokens: provisions a loginable vendor before
        exercising refresh.
        - test_refresh_rejects_invalid_tokens: supplies the shared client id
        used by refresh route requests.
        - test_valid_token_returns_vendor_id: creates the vendor used to access
...
def signup(client: TestClient, payload: dict[str, str]):
    ...
    Used by:
        - test_signup_creates_vendor_201: exercises the happy-path signup
        contract.
        - test_signup_duplicate_email_returns_409: performs the first and second
        signup attempts.
        - test_login_returns_token_pair: provisions a vendor before login.
        - test_login_rejects_invalid_credentials: creates the baseline vendor
        record.
        - test_refresh_issues_new_tokens: creates the vendor whose refresh token
        is later exchanged.
        - test_refresh_rejects_invalid_tokens: creates a vendor when the
        invalid-token scenario still requires existing state.
        - test_valid_token_returns_vendor_id: provisions the vendor used for the
    ...
    Returns:
        - Response: The raw FastAPI test-client response from
...
def login(client: TestClient, payload: dict[str, str]):
    ...
    Used by:
        - test_login_returns_token_pair: exercises the happy-path login
        contract.
        - test_login_rejects_invalid_credentials: submits invalid credentials
        against the real route.
        - test_refresh_issues_new_tokens: obtains the refresh token used for the
        follow-up refresh call.
        - test_valid_token_returns_vendor_id: obtains the access token used for
        the protected endpoint call.
...
def test_signup_creates_vendor_201(faker) -> None:
    """
    Verifies that `api.routes.auth.signup_route` creates a vendor and matters
    because the public auth API must expose the persisted vendor identity after
    a successful signup.
...
def test_signup_duplicate_email_returns_409(faker) -> None:
    ...
    because duplicate vendor creation must fail at the API boundary rather than
    silently creating competing accounts.
...
def test_signup_validation_errors_return_422(faker, scenario: str) -> None:
    ...
    errors. This matters because the route contract must reject invalid client
    input consistently.
    ...
    Parametrize:
        ...
        - Cases:
            - <id="weak_password"> — password is shorter than the signup policy
            permits.
...
def test_login_returns_token_pair(faker) -> None:
    """
    Verifies that `api.routes.auth.login_route` returns an access token and a
    refresh token. This matters because clients depend on the login route to
...
def test_login_rejects_invalid_credentials(
    ...
    wrong-password attempts. This matters because the route must not
    authenticate invalid credentials regardless of which field is wrong.
    ...
    Parametrize:
        Cases:
            ...
            - <id="wrong_password"> — submits the persisted email with the wrong
            password.
    """
    payload = build_auth_payload(faker)
...
def test_refresh_issues_new_tokens(faker) -> None:
    ...
    token for a new token pair. This matters because authenticated clients rely
    on refresh to continue operating without re-entering credentials.
    ...
    Fixtures:
        - faker: Session-scoped `Faker` instance used to generate the
        signup/login payload and client id.
    """
    payload = build_auth_payload(faker)
    with TestClient(app) as client:
        signup(client, payload)
...
def test_refresh_rejects_invalid_tokens(
    ...
    """
    Verifies that `api.routes.auth.refresh_route` rejects invalid tokens. This
    ...
    Fixtures:
        - app_settings: Shared `Settings` object containing the signing secret used
        to mint the invalid token variants.
        - faker: Session-scoped `Faker` instance used to generate payload values and
        the garbage token string.

    Parametrize:
        - <id="access_token"> — uses a real access token where a refresh token
        is required.
        - <id="garbage_token"> — uses an unsigned arbitrary string that should
        fail decoding.
    """
    payload = build_auth_payload(faker)
    invalid_token = (
