...
def test_health_check_returns_ok():
    """
    Purpose:
        Verifies that `app.api.routes.health.health_check` returns the public health payload with an OK status and timestamp.
        This matters because infrastructure and uptime checks depend on a stable liveness contract.
...
def test_health_check_has_request_id():
    """
    Purpose:
        Verifies that `app.api.routes.health.health_check` responses include the request-id header added by the middleware stack.
        This matters because error and success responses should be traceable with the same correlation identifier mechanism.
